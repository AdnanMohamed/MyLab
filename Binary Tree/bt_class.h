...
#include "binary_tree.h"  // Provides binary_tree_node<Item> template class
...
namespace binary_tree_adnan
{
    ...
    public:
        ...
        ~binary_tree() { tree_clear(root_ptr); }
        ...
        Item retrieve() const;
        bool has_parent() const { return size() > 0 && current_ptr != root_ptr; }
        bool has_left_child() const { return size() > 0 && current_ptr->left() != NULL;}
        bool has_right_child() const { return size() > 0 && current_ptr->right() != NULL; }

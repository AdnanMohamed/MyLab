...
#include"../Employee class/Employee.cpp"

namespace salaried_employee_adnan
{
	using namespace employees_adnan;
	...
	std::ostream& operator<<(std::ostream& outs, const Salaried_Employee& emp)
	{
		...
		outs.precision(2);
		outs << Employee(emp);
		outs << "Monthly Salary: $" << emp.salary_ << endl;
		outs << "Annual Salary: $" << emp.get_annual_salary() << endl;
		outs << "Contract Working Hours (per month): " << emp.hours_per_month_ <<" hours/ month"<< endl;
		double extra_hours = emp.hours_worked_ - emp.hours_per_month_;
		if (extra_hours > 0)
		{
			outs << "Overtime Hours: " << extra_hours << " hours" << endl;;
			outs << "Overtime Pay: $" << emp.overtime() << endl;
		}
		outs << "------------------------\n";
		return outs;
	}
